...
---@field follow_selection boolean?
...
M.setup = function(config, global_config)
  ...
  -- Set up follow_selection: preview symbol on cursor move in document_symbols buffer
  if config.follow_selection then
    local group = vim.api.nvim_create_augroup("neo_tree_document_symbols_follow", { clear = true })
    manager.subscribe(M.name, {
      ...
      handler = function()
        ...
        vim.api.nvim_create_autocmd("CursorMoved", {
          ...
          callback = function()
            ...
            commands.preview_symbol(current_state)
          end,
        })
      end,
    })
